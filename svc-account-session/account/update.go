...
func (e *ExternalInterface) Update(ctx context.Context, req *accountproto.UpdateAccountRequest, session *asmodel.Session) response.RPC {
			...
			if requestUser.RoleID != common.RoleMonitor {
				if requestUser.RoleID != common.RoleClient {
					...
					if err != nil {
						...
						args := getResponseArgs(resp, errorMessage, []interface{}{requestUser.RoleID, "RoleID"})
						resp.Body = args.CreateGenericErrorResponse()
						l.LogWithFields(ctx).Error(errorMessage)
						return resp
					}
	...
	if gerr != nil {
		...
		if errors.DBKeyNotFound == gerr.ErrNo() {
			...
			args := getResponseArgs(resp, errorMessage, []interface{}{"Account", id})
			resp.Body = args.CreateGenericErrorResponse()
		} else {
	...
	if user.UserName != session.UserName && !session.Privileges[common.PrivilegeConfigureUsers] {
		...
		args := getResponseArgs(resp, errorMessage, []interface{}{})
		resp.Body = args.CreateGenericErrorResponse()
	...
	if requestUser.Password != "" {
		...
		if err = validatePassword(user.UserName, requestUser.Password); err != nil {
			...
			args := getResponseArgs(resp, errorMessage, []interface{}{requestUser.Password, "Password"})
			resp.Body = args.CreateGenericErrorResponse()
			l.LogWithFields(ctx).Error(errorMessage)
			return resp
		}
...
func getResponseArgs(resp response.RPC, errorMessage string, messageArgs []interface{}) response.Args {
